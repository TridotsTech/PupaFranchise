...
def execute(filters=None):
	...
	for sle in sl_entries:
		...
		if sle.voucher_type == "Stock Entry" and sle.voucher_no:
			se_type = frappe.db.get_value("Stock Entry", sle.voucher_no, "stock_entry_type")
			sle["stock_entry_type"] = se_type
		if sle.voucher_type in ["Delivery Note", "Sales Invoice"]:
			if sle.voucher_type == "Delivery Note":
				dn_doc = frappe.get_doc("Delivery Note", sle.voucher_no)
				if dn_doc and dn_doc.customer:
					sle["customer"] = dn_doc.customer
			elif sle.voucher_type == "Sales Invoice":
				si_doc = frappe.get_doc("Sales Invoice", sle.voucher_no)
				if si_doc and si_doc.customer:
					sle["customer"] = si_doc.customer
		# thirvusoft edit end
